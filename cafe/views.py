...
def generate_bill(request):
    ...
    if not t_number:
        return HttpResponse("No table specified", status=400)

    order_for_table = order.objects.filter(table=t_number, bill_clear=False)

    if not order_for_table.exists():
        return HttpResponse("No active orders for this table", status=404)
    ...
    for item in bill_items:
        order_items = json.loads(item['order_items'])
        for _, pr_value in order_items.items():
            key = pr_value[1].lower()
            qty = pr_value[0]
            price = pr_value[2]
            order_dict[key] = [qty, qty * price]
